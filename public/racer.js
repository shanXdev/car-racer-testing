function racer(gamemode) {
  ...
  function update(dt) {
    ...
    if (turboTriggered && gamemode == 1) {
      ...
      playerX =
        playerX - dx * speedPercent * playerSegment.curve * turboCentrifugal;
    } else {
    ...
    if (!gameOverFlag) {
      ...
      // game over flag, just decelerate the car until full stop
    ...
    if (position > playerZ) {
      if (currentLapTime && startPosition < playerZ) {
        // arrived at finish line, update last lap time + generate new track if enabled
        if (gamemode == 1) {
          ...
          if (currentLevel >= difficultyMax) {
            ...
            if (randomTrack) {
              ...
              if (
                (difficultyCurrent % difficultyGap == 0) &
                (difficultyCurrent < difficultyMax)
              ) {
            ...
            if (
              (changeBackgroundEvery > 0) &
              (currentLevel % changeBackgroundEvery == 0)
            ) {
          ...
          // fastest lap time gamemode
          lastLapTime = currentLapTime;
          currentLapTime = 0;
          if (
            (lastLapTime <= Util.toFloat(Dom.storage.fast_lap_time)) |
            (Util.toFloat(Dom.storage.fast_lap_time) == 0)
          ) {
        ...
        if (remainingTime > 0) {
          ...
          if (currentLevel == 0) {
            ...
            remainingTime +=
              trackLength * remainingTimeIncrease * remainingTimeStartBonus;
          }
        }
        ...
        if ((gamemode == 1) & (remainingTime <= 0)) {
          // gamemode out of time and no remaining time left
          gameOverFlag = true;
        }
      }
    }
  ...
  function render() {
    ...
    if (!changeBackgroundFlag) {
      ...
      Render.background(
        ...
        skyOffset,
        resolution * skySpeed * playerY,
        1.0 - changeBackgroundCurrentAlpha
      );
      Render.background(
        ctx,
        background,
      ...
      Render.background(
        ...
        width,
        height,
        background_pos_next[0],
        skyOffset,
        resolution * skySpeed * playerY,
        changeBackgroundCurrentAlpha
      );
      Render.background(
        ctx,
        background,
    ...
    if (enableTilt) {
      ...
      if (baseSegment.curve == 0) {
        ...
        newrot =
          Math.round(((baseSegment.curve * speed) / maxSpeed) * 100) / 100;
  ...
  function resetRoad(random, mintracklength) {
    ...
    if (random == true) {
      ...
      while ((i += 1) < mintracklength) {
        // TODO: sometimes, 2 tracks are not enough and the loading fails, try to find out why? (there must be an incompatibility between constructs somewhere)
    ...
    try {
      // workaround for the exception raised sometimes when the tracklength is too small (<5). TODO: fix the root cause of this.
  ...
  function resetCars() {
    ...
    for (var n = 0; n < totalCars; n++) {
      ...
      z =
        Math.floor(Math.random() * (segments.length - 400)) * segmentLength +
        100 * segmentLength; // ensure that cars do not respawn just in front of the player, so we generate cars in all segments except the first and last (after and before finish line) - TODO: fix me in a more elegant way, without using constants
